...
class TicketModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction):
        ...
        for role_id in SUPPORT_ROLE_IDS:
            role = guild.get_role(role_id)
            if role:
                await channel.send(f"{role.mention} - New ticket!", delete_after=30)
...
class TicketPanelView(discord.ui.View):
    ...
    @discord.ui.button(label="Deutsch", style=discord.ButtonStyle.primary, custom_id="lang_de", emoji="🇩🇪")
    async def de_button(self, interaction, button):
        view = PrioritySelectionView(lang="de")
        await interaction.response.send_message("Bitte waehle die Prioritaet:", view=view, ephemeral=True)

    @discord.ui.button(label="English", style=discord.ButtonStyle.primary, custom_id="lang_en", emoji="🇬🇧")
    async def en_button(self, interaction, button):
        view = PrioritySelectionView(lang="en")
        await interaction.response.send_message("Please select priority:", view=view, ephemeral=True)
...
async def on_ready():
    ...
    if guild:
        category = guild.get_channel(TICKET_CATEGORY_ID)
        if category:
            ticket_counter = len([c for c in category.channels if 'ticket-' in c.name.lower()])
            logger.info(f"Initialized ticket_counter to {ticket_counter}")
...
async def panel_command(interaction):
    ...
    embed = discord.Embed(
        ...
        description="Waehle deine Sprache / Choose your language",
...
async def sla_check():
    now = datetime.now(timezone.utc)
    for ch_id, td in list(ticket_data.items()):
        if td.get('status') == 'open':
            ...
            if elapsed > SLA_MINUTES:
                ...
                if channel:
                    await channel.send(f"WARNING: SLA breach! {elapsed:.0f} min elapsed.")
                    await api_request('POST', '/bot/event', {'event_type': 'sla_breach', 'data': {'ticket_id': td['channel_id']}})
...
async def auto_close_check():
    now = datetime.now(timezone.utc)
    for ch_id, td in list(ticket_data.items()):
        if td.get('status') in ['open', 'claimed']:
            created = datetime.fromisoformat(td['created_at'])
            ...
            if elapsed_hours > AUTO_CLOSE_HOURS:
                ...
                if channel:
                    ...
                    await asyncio.sleep(5)
                    await channel.delete(reason="Auto-closed")
                    if td.get('user_id'):
                        uid = int(td['user_id'])
                        if uid in user_tickets and ch_id in user_tickets[uid]:
                            user_tickets[uid].remove(ch_id)
