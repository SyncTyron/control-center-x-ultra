...
async def bot_push_event(event: BotEventCreate, request: Request):
    ...
    # Update ticket in database based on event type
    if event.event_type == "ticket_claim":
        ticket_id = event.data.get("ticket_id")
        claimed_by = event.data.get("claimed_by")
        if ticket_id and claimed_by:
    ...
    elif event.event_type == "ticket_close":
        ticket_id = event.data.get("ticket_id")
        closed_by = event.data.get("closed_by")
        if ticket_id and closed_by:
            await db.tickets.update_one(
                {"channel_id": ticket_id},
                {"$set": {
                    "closed_by": closed_by,
...
async def bot_store_message(msg: BotMessageCreate, request: Request):
    bot_token = request.headers.get("X-Bot-Token", "")
    if bot_token != DISCORD_BOT_TOKEN:
        raise HTTPException(status_code=403, detail="Invalid bot token")
