"""
Enterprise Discord Ticket Bot - Control Center X Ultra - FIXED VERSION
"""
...
class PrioritySelect(discord.ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(label="Low", value="low", description="Niedrige Priorität", emoji="🟢"),
            discord.SelectOption(label="Medium", value="medium", description="Mittlere Priorität", emoji="🟡"),
            discord.SelectOption(label="High", value="high", description="Hohe Priorität", emoji="🟠"),
            discord.SelectOption(label="Critical", value="critical", description="Kritisch / Dringend", emoji="🔴"),
        ]
        super().__init__(placeholder="Priorität wählen / Select Priority", min_values=1, max_values=1, options=options)
...
class TicketModal(discord.ui.Modal):
    def __init__(self, lang: str, priority: str):
        self.lang = lang
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        for ch_id, td in ticket_data.items():
            if td['user_id'] == str(user.id) and td['subject'].lower() == self.subject.value.lower():
        ...
        # Create ticket channel with new naming format: {lang}.ticket.{priority}.{number}
        ...
        # NEW NAMING FORMAT: {lang}.ticket.{priority}.{number}
        lang_prefix = "de" if self.lang == "de" else "en"
        channel_name = f"{lang_prefix}-ticket-{priority}-{ticket_num:04d}"
        ...
        # Store ticket data - FIX: user_id as STRING
        created_at = now.isoformat()
        td = {
            'channel_id': str(channel.id),
            'guild_id': str(guild.id),
            'user_id': str(user.id),  # FIX: Convert to string!
...
# --- Priority Selection View (before Modal) ---
class PrioritySelectionView(discord.ui.View):
    ...
    @discord.ui.button(label="Weiter / Continue", style=discord.ButtonStyle.success, row=1)
    async def continue_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(TicketModal(lang=self.lang, priority=self.priority))
...
class TicketActions(discord.ui.View):
    ...
    async def close_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if user_id:
            ...
            if user_id_int in user_tickets and interaction.channel.id in user_tickets.get(user_id_int, []):
                user_tickets[user_id_int].remove(interaction.channel.id)
        # Rename channel
    ...
    @discord.ui.button(label="Reopen", style=discord.ButtonStyle.secondary, custom_id="ticket_reopen", emoji="🔓")
    async def reopen_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if td:
            ...
            if user_id:
                user_id_int = int(user_id)
                user_tickets.setdefault(user_id_int, []).append(interaction.channel.id)
...
class TicketPanelView(discord.ui.View):
    ...
    async def de_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Show priority selection first
        view = PrioritySelectionView(lang="de")
        await interaction.response.send_message(
            "**Wähle die Priorität deines Tickets:**\n"
            "🟢 Low - Niedrig\n🟡 Medium - Mittel\n🟠 High - Hoch\n🔴 Critical - Kritisch",
            view=view,
            ephemeral=True
        )
...
async def ticket_panel(interaction: discord.Interaction):
    ...
    embed.add_field(name="🇩🇪 Deutsch", value="Klicke für deutschen Support", inline=True)
    embed.add_field(name="🇬🇧 English", value="Click for English support", inline=True)
    ...
    for channel_id, td in list(ticket_data.items()):
        ...
        if not td.get('claimed_by') and age_minutes > SLA_MINUTES:
            if not td.get('sla_warned'):
                td['sla_warned'] = True
                embed = discord.Embed(
                    description=f"⚠️ SLA Warning: This ticket has been open for {int(age_minutes)} minutes without response!",
